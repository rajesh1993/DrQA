...
# )
predictor = Predictor(
    model='/home/sai/NLP/Project/DrQA/data/reader/multitask.mdl',
    tokenizer='corenlp',
    embedding_file=None,
    num_workers=1
)
...
f_qa_json = open('/home/sai/NLP/Project/DrQA/data/MovieQA_benchmark/data/qa_modified_options.json', 'r')
...
for i, qid, question, imdb_key, candidates in qn_set_df.loc[:10, ['qid', 'question', 'imdb_key', 'answers']].itertuples():
    plot_location = mv_set_df.loc[mv_set_df['imdb_key'] == imdb_key]['text'][0]['plot']
    if plot_location is not None:
        plot_file = open(os.path.join(os.getcwd(), 'data/MovieQA_benchmark/'+plot_location), 'r')
        context = plot_file.read(16384)
        plot_file.close()
        qids.append(qid)
...
# print('Sorted results: {}'.format(sorted(results.items())))
